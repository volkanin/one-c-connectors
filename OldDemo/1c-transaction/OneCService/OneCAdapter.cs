...
{	
	public class OneCAdapter : IDisposable
	{
		...
		private void RestoreConnectionRCW()
		{
			try
			{							
				object rcw = Marshal.GetObjectForIUnknown(connectionPtr);
				connection = rcw;
			}
			catch (Exception _e)
			{		
				SimpleLogger.DefaultLogger.Severe("Error in ResotoreConnectionRCW: "+_e.ToString());
			}
		}
		...
		{			
			...
			//Чтобы не всегда иметь возможность восстановить RCW			
			//GCHandle gch = GCHandle.Alloc(connection);
			//connectionPtr = gch.AddrOfPinnedObject();
		...
		public void Begin()
		{
			...
			if (this.inTransactionFlag == false)
			{
				Invoke(connection, "НачатьТранзакцию", new object[] {});					
		...
		{
			RestoreConnectionRCW();
			Invoke(connection, "ЗафиксироватьТранзакцию", new object[] {});
		...
		{
			RestoreConnectionRCW();
			Invoke(connection, "ОтменитьТранзакцию", new object[] {});
			this.inTransactionFlag = false;
		}
